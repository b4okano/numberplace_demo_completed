...
def detect_corners(raw_img):
    img = fit_size(raw_img, 500, 500) # 入力画像を500x500以内にリサイズ
    polies = convex_poly(img)          # 輪郭を抽出し、凸包→多角形近似→多角形リスト
    # print(polies)
    poly = select_corners(img, polies)  # 四角形の中で「正方形に最も近いもの」を選ぶ
    if poly is None:     #四隅が検出できなかった場合Noneを返す
        return None,None
    else:
        x0 = poly.flatten()                  # flat化（後で使うがこの関数では未使用）
        rect, score = convex_poly_fitted(img)  # さらに精密に最適化して四角形検出
        print(rect)
        # スケールを元画像に合わせて戻す
...
def contours(img, show=True):
    ...
    #0.2 元の値 画像サイズに占める割合の閾値
    min_area = img.shape[0] * img.shape[1] * 0.15
    # 画像の中の特定の割合を占める輪郭だけを取得。
...
def convex_poly(img, show=True):
    cnts = convex(img, False) # 輪郭を取得し、凸包化されたリストを取得
    blank = np.copy(img)
    polies = []
    for cnt in cnts:
...
def select_corners(img, polies):
    ...
    for poly in polies:
        ...
        # 正方形に近いものを選ぶ
        for c in choices:
            #正方形は等価の辺の掛け算が面積になることを応用し、面積**0.5と持ってきた輪郭の
            # 一辺がどれだけ近いかを比較する。
            ...
            scores.append(score)
        idx = np.argmin(scores) #一番誤差が小さいものを
