...
def add_border_lines(img, line_num=3, border_width=5):
    """
    img: (28,28) 正規化済み画像 (0~1)
    line_num: 線の本数
    border_width: 外枠の範囲（ピクセル）
    """
    ...
    for _ in range(line_num):
        ...
        def random_border_point():
            ...
            if side == 'top':
                return (np.random.randint(0, w), np.random.randint(0, border_width))
            elif side == 'bottom':
                return (np.random.randint(0, w), np.random.randint(h - border_width, h))
            elif side == 'left':
                return (np.random.randint(0, border_width), np.random.randint(0, h))
            else:  # right
                return (np.random.randint(w - border_width, w), np.random.randint(0, h))
        
        pt1 = random_border_point()
        pt2 = random_border_point()
        color = 255  # 白線
...
def dataset_line(add_border_noise=False, line_num=3, border_width=5):
    # MNIST読み込み
    (x_train, y_train), (x_test, y_test) = keras.datasets.mnist.load_data()
